...
% UTILIZZAZIONE DI TRE FEATURES PER LA CLASSIFICAZIONE
% 
% ASCOLTO DEI SEGNALI:
...
%fasi
den=1/340*0.04;   %1/c^(-1)*d    c=velocità prop.  d=distanza microfoni
fasi=fasi/den;

%creo gli altri due vettori di features
mod1=abs(STFT1)/sqrt(sum(sum(abs(STFT1))));
mod2=abs(STFT2)/sqrt(sum(sum(abs(STFT2))));


fasi=reshape(fasi,size(fasi,1)*size(fasi,2),1);
mod1=reshape(mod1,size(mod1,1)*size(mod1,2),1);
mod2=reshape(mod2,size(mod2,1)*size(mod2,2),1);


matrix=[mod1,mod2,fasi];

%Utilizzo una metrica in norma L1 per "proteggermi" da errori di stima
%degli outliers presenti
[IDX Cen]= kmeans(matrix,3);     %vedi diverse metriche di distanza
...
figure(5); imagesc(10*log10(abs(tra1))); title('Prima Sorgente ricostruita');
figure(6); imagesc(10*log10(abs(tra2))); title('Seconda Sorgente ricostruita');
figure(7); imagesc(10*log10(abs(tra3))); title('Terza Sorgente ricostruita')
